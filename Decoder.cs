...
namespace RS41Decoder
{
    public class Decoder
    {
        ...
        private const int NDATA_LENGTH = 320; // std framelen 320
        private const int XDATA_LENGTH = 198;
        private const int FRAME_LENGTH = NDATA_LENGTH + XDATA_LENGTH; // max framelen 518
        ...
        private const int pos_AUX = 0x12B;
        ...
        public void DecodeLoop(BinaryReader reader, Demodulation demod)
        {
            ...
            while (true)
            {
                Tuple<int, int> bits = demod.ReadBits(reader);

                if (bits.Item2 == 0)
                {
                    if (byteCount > pos_AUX)
                    {
                        DecodeFrame();
                        bitCount = 0;
